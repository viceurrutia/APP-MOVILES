...
object Prefs{
 ...
 fun saveProfile(ctx:Context, email:String, map:Map<String,String>){ val key="profile_$email"; val flat=map.entries.joinToString("&"){ "${it.key}=${it.value.replace("&"," ")}" }; p(ctx).edit().putString(key, flat).apply() }
 fun getProfile(ctx:Context, email:String):Map<String,String>{ val key="profile_$email"; val raw=p(ctx).getString(key,"")?:return emptyMap(); if(raw.isEmpty()) return emptyMap(); return raw.split("&").mapNotNull{ val parts=it.split("="); if(parts.size==2) parts[0] to parts[1] else null }.toMap() }
}